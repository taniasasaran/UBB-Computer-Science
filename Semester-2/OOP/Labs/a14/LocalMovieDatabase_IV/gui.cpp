...
void GUI::connect_signals_and_slots(){
    ...
    QObject::connect(admin_window->undo_button, &QPushButton::clicked, this, &GUI::undo_admin_handler);
    QKeySequence undo_shortcut = QKeySequence("Ctrl+Z");
    admin_window->undo_button->setShortcut(undo_shortcut);
    QObject::connect(admin_window->undo_button, &QPushButton::toggled, this, &GUI::undo_admin_handler);
    QObject::connect(admin_window->redo_button, &QPushButton::clicked, this, &GUI::redo_admin_handler);
    QKeySequence redo_shortcut = QKeySequence("Ctrl+Y");
    admin_window->redo_button->setShortcut(redo_shortcut);
    QObject::connect(admin_window->redo_button, &QPushButton::toggled, this, &GUI::redo_admin_handler);
...
void GUI::update_admin_handler() {
    ...
    validate.check_integer(number_of_likes);
    validate.check_year_greater_1000(year_of_release);
    Movie movie_updated(movie.getTitle(), movie.getGenre(), std::stoi(year_of_release), std::stoi(number_of_likes), trailer);
    bool ok=false;
    for(auto m: this->service.getRepoData())
        if(m.getGenre() == movie.getGenre() && m.getTitle()==movie.getTitle())
            ok=true;
    if(ok) {
        this->service.update(movie.getTitle(), movie.getGenre(), movie_updated);
        init_admin_window();
    }
    else
        oops_an_error_occurred();
...
void GUI::undo_admin_handler() {
    int result = service.undo();
    if(result)
        oops_an_error_occurred();
    init_admin_window();
}

void GUI::redo_admin_handler() {
    int result = service.redo();
    if(result)
        oops_an_error_occurred();
    init_admin_window();
}
